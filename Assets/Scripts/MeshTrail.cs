...
public class MeshTrail : MonoBehaviour
{
    ...
    {   
        normalSpeed = moveScript.movementSpeed;             //현재 속도를 저장하고 증가된 속도 사용
        ...
        while(timeActivated > 0)
        {
            ...
            if(skinnedRenderer == null)                     //스킨드 메시 랜더러 컴포넌트들을 가져옴
                skinnedRenderer = positionToSpawn.GetComponentsInChildren<SkinnedMeshRenderer>();

            for(int i = 0; i < skinnedRenderer.Length; i++) //각 메시 랜더러에 대한 잔상 생성
            {
                ...
                Destroy(g0bj, meshDestroyDelay);            //일정 시간 후 잔상 제거
            }
            //다음 잔상 생성까지 대기
